    ...
    <div className="max-w-2xl mx-auto p-6 mt-8">
      ...
      {sessions.length === 0 ? (
        <p className="text-slate-500 text-sm">
          No sessions yet.{isAdmin ? " Create one to build a workout." : " Ask an admin to set one up."}
        </p>
      ) : (
        <div className="flex flex-col gap-2">
          {sessions.map((s) => (
            ...
            >
              <div>
                <span className="text-slate-100 font-medium">{s.name || "Untitled"}</span>
                <span className="text-slate-600 text-xs ml-3">{s.item_count} items</span>
